...
public class RestaurantApprovalResponseKafkaListener implements
  ...
  public void receive(
    ...
    log.info("'{}' number of restaurant responses received with keys '{}, partitions '{}' " +
            "and offset '{}'", messages.size(), keys.toString(), partitions.toString(),
        offsets.toString());

    messages.forEach(message -> {
      switch (message.getOrderApprovalStatus()) {
        case APPROVED -> {
          log.info("Processing approved order for order id '{}'", message.getOrderId());
          listener.orderApproved(mapper.approvalResponseAvroModelToApprovalResponse(message));
        }
        case REJECTED -> {
          log.info("Processing rejected order for order id '{}', with failure messages: '{}'",
              message.getOrderId(), String.join(",", message.getFailureMessages()));
          listener.orderRejected(mapper.approvalResponseAvroModelToApprovalResponse(message));
        }
      }
    });
  }
